...
export const ContractorSection = () => {
    ...
    useEffect(() => {
        const fetchContractors = async () => {
            try {
                const res = await api.get('/api/hr/public');
                if (res.data?.success) {
        ...
        <section className="py-32 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 transition-colors duration-300">
            <div className="max-w-7xl mx-auto px-6">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-12 text-[var(--text-secondary)]">Không có nhà thầu công khai.</div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        {contractors.map((contractor, idx) => (
                            ...
                            >
                                <Link href={`/hr/${contractor.id}/profile`} className="block">
                                    <div className="flex items-center gap-6 mb-10">
                                        <div className="w-20 h-20 rounded-2xl bg-primary/10 flex items-center justify-center overflow-hidden">
                                            {contractor.logo ? (
                                                <img src={contractor.logo} alt={contractor.name} className="w-full h-full object-cover" />
                                            ) : (
                                                <span className="material-symbols-outlined text-primary text-5xl">{contractor.icon || "business"}</span>
                                            )}
                                        </div>
                                        <div>
