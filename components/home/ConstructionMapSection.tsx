      ...
      >
        ...
        <AnimatePresence>
          {isHovered && !isActive && (
            <motion.div
              ...
              className="absolute bottom-full mb-3 px-3 py-1.5 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-xs font-bold rounded-lg shadow-xl whitespace-nowrap border border-slate-200 dark:border-slate-700 z-10"
            >
              {site.name}
        ...
        <motion.div
          ...
          className={`relative w-11 h-11 rounded-full flex items-center justify-center transition-all duration-500 border-2 ${isActive
            ...
            : "bg-slate-800 dark:bg-slate-800 border-white/20 group-hover:border-white/40 shadow-xl"
            }`}
        >
        ...
        {/* Pointer shadow */}
        <div className="w-1.5 h-1.5 bg-slate-900/20 dark:bg-white/20 rounded-full blur-[1px] mt-1" />
      </div>
    </Marker>
  );
};
...
const SitePopupCard = ({ site, onClose }: { site: Site; onClose: () => void }) => {
  return (
    ...
    >
      <div className="p-5 bg-white dark:bg-slate-950/90 backdrop-blur-2xl text-slate-900 dark:text-white rounded-[2rem] border border-slate-200 dark:border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] min-w-[280px]">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="min-w-0">
            <h4 className="font-black text-base leading-tight text-slate-900 dark:text-white truncate">
              {site.name}
            </h4>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-bold flex items-center gap-1 opacity-80">
        ...
        <div className="space-y-4">
          <div className="bg-slate-50 dark:bg-slate-900/50 p-3 rounded-2xl border border-slate-100 dark:border-white/5 space-y-2">
            <div className="flex items-center justify-between text-[11px] font-black">
              <span className="text-slate-400 dark:text-slate-500 uppercase tracking-widest">Construction Progress</span>
          ...
          <button className="w-full py-3.5 bg-slate-900 dark:bg-white text-white dark:text-slate-950 hover:bg-secondary hover:text-slate-900 text-[11px] font-black uppercase tracking-[0.15em] rounded-2xl transition-all shadow-xl active:scale-95 border border-transparent">
        ...
        <div className="flex flex-col lg:flex-row items-stretch gap-12 mb-14">
          ...
          <div className="lg:w-1/3 flex flex-col shrink-0">
            ...
            <motion.h2
              ...
              viewport={{ once: true }}
              className="text-4xl md:text-5xl font-display font-black text-slate-900 dark:text-white mb-6 leading-tight"
            >
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left flex items-start gap-4 rounded-3xl p-5 transition-all duration-500 border group ${isActive
                      ? "bg-white dark:bg-slate-800 border-secondary shadow-2xl ring-1 ring-secondary/20 -translate-y-1"
                      : "bg-white dark:bg-slate-800/40 border-slate-200 dark:border-white/5 hover:border-secondary/30 dark:hover:border-white/10 hover:bg-slate-50 dark:hover:bg-slate-800/60 shadow-sm hover:shadow-md"
                      }`}
                  >
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-colors ${isActive ? "bg-secondary text-slate-900" : "bg-slate-100 dark:bg-slate-700/50 text-slate-400 group-hover:text-secondary"
                      }`}>
                      <span className="material-symbols-outlined">apartment</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <p className={`font-black text-base truncate transition-colors ${isActive ? "text-slate-900 dark:text-white" : "text-slate-700 dark:text-slate-200"}`}>
                          {site.name}
                        </p>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1 h-1.5 rounded-full bg-slate-100 dark:bg-slate-950 overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${site.progress}%` }}
                            className={`h-full ${isActive ? "bg-secondary" : "bg-slate-300 dark:bg-slate-600"}`}
                          />
                        </div>
                        <span className={`text-[10px] font-black whitespace-nowrap ${isActive ? "text-secondary" : "text-slate-400 dark:text-slate-500"}`}>
          ...
          >
            <div className="relative h-full min-h-[500px] lg:min-h-0 rounded-[3.5rem] overflow-hidden bg-white dark:bg-slate-950 border border-slate-200 dark:border-white/10 shadow-[0_0_100px_-20px_rgba(0,0,0,0.1)] dark:shadow-[0_0_100px_-20px_rgba(0,0,0,0.8)] group transition-all duration-500">
              <div className="absolute inset-0 rounded-[3.5rem] pointer-events-none ring-1 ring-inset ring-slate-900/5 dark:ring-white/10 z-10" />
              ...
              {MAPBOX_TOKEN ? (
                <>
                  <Map
                    ...
                    mapStyle={`mapbox://styles/mapbox/navigation-${isDarkMode ? 'night' : 'day'}-v1`}
                  ...
                  <div className="absolute inset-x-0 bottom-0 h-48 bg-gradient-to-t from-white dark:from-slate-950 via-white/40 dark:via-slate-950/40 to-transparent pointer-events-none z-[5] transition-colors duration-500" />
                  <div className="absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-white/60 dark:from-slate-950/60 to-transparent pointer-events-none z-[5] transition-colors duration-500" />
                  ...
                  <div className="absolute top-8 left-8 pointer-events-none z-20">
                    <motion.div
                      ...
                      className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-2xl px-6 py-4 rounded-[2rem] border border-slate-200 dark:border-white/10 shadow-xl dark:shadow-2xl transition-colors duration-500"
                    >
                      <p className="text-[11px] font-black text-slate-900 dark:text-white/90 flex items-center gap-3">
                  ...
                  <div className="absolute bottom-10 left-8 pointer-events-none z-20">
                    <motion.div
                      ...
                      className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl px-4 py-2.5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-lg flex items-center gap-4 transition-colors duration-500"
                    >
                  ...
                  <button
                    ...
                    className="absolute top-8 right-16 z-20 flex items-center gap-2 bg-slate-900 dark:bg-white text-white dark:text-slate-950 text-[11px] font-black uppercase tracking-[0.15em] px-5 py-3.5 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.4)] hover:bg-slate-800 dark:hover:bg-secondary dark:hover:text-slate-900 transition-all transform hover:-translate-y-1 active:scale-95"
                  >
      ...
      <AnimatePresence>
        {MAPBOX_TOKEN && isMapModalOpen && (
          ...
          >
            ...
            >
              <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] border border-slate-200 dark:border-white/10 overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.2)] dark:shadow-[0_0_100px_rgba(0,0,0,0.5)] h-full relative group">
                <Map
                  ref={modalMapRef}
                  initialViewState={initialViewState}
                  mapStyle={`mapbox://styles/mapbox/navigation-${isDarkMode ? 'night' : 'day'}-v1`}
                ...
                <button
                  type="button"
                  onClick={() => setIsMapModalOpen(false)}
                  className="absolute top-8 right-16 w-12 h-12 rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all z-20"
                >
